...
#define FRAME_COUNT 5
	...
	{
		#include "20170301_HX83100-A_Innolux_FW_02_00_09_0D.i"
	};
#endif
...
static void lct_tp_info(char *module)
{
   ...
   snprintf(info, sizeof(info), "[fw]0x%4.4x,[ic]hx83100", ic_data->vendor_fw_ver); 
   snprintf(dev_info, sizeof(dev_info), "[Vendor]%s,[fw]0x%4.4x,[ic]hx83100", module, ic_data->vendor_fw_ver);
   if(!module)
...
//	uint8_t tmp_data[4];
	...
	if (( ic_data->vendor_fw_ver < i_FW_VER ) || ( ic_data->vendor_config_ver < i_CFG_VER ))
		{
			himax_int_enable(private_ts->client->irq,0);
update_retry:
...
void himax_ts_work(struct himax_ts_data *ts)
{
	...
	if (check_sum_cal != 0 && HX_ESD_RESET_ACTIVATE == 0)
	{
