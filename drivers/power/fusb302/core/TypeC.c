...
void ConfigurePortType(FSC_U8 Control)
{
    ...
    if (PortType != value)
    {
    printk("fusb302 func=%s,line=%d,PortType=%d,value=%d\n",__func__,__LINE__,PortType,value);
        switch (value)
        {
            case 1:
#ifdef FSC_HAVE_SRC
    ...
    if (SourceCurrent != value){
        switch (value)
        {
        printk("fusb302 func=%s,line=%d,SourceCurrent=%d,value=%d\n",__func__,__LINE__,SourceCurrent,value);
            case 1:
                SourceCurrent = utccDefault;
                break;
            case 2:
...
FSC_U8 GetTypeCSMControl(void)
{
    ...
    {
    printk("fusb302 func=%s,line=%d,PortType=%d\n",__func__,__LINE__,PortType);
...
{
    platform_set_vbus_lvl_enable(VBUS_LVL_ALL, FALSE, FALSE);       // Disable the vbus outputs
    if (!IsHardReset)                                               // LONGCHEER, P3592
		PolicyIsDFP = FALSE;                                        // LONGCHEER, P3592
	pr_info("FUSB PolicyIsDFP = %d, %s\n", PolicyIsDFP, __func__);  // LONGCHEER, P3592
	USBPDDisable(TRUE);  
...
void ProcessTypeCPDControl(FSC_U8* MsgBuffer, FSC_U8* retBuffer)
{
    ...
    {
     printk("fusb302 func=%s,line=%d,MsgBuffer[4]=0x%0x\n",__func__,__LINE__,MsgBuffer[4]);
...
void ProcessSetTypeCState(FSC_U8* MsgBuffer, FSC_U8* retBuffer)   // Set state machine
{
    ...
    switch(state)
    {
     printk("fusb302 func=%s,line=%d,state=%d\n",__func__,__LINE__,state);
        case(Disabled):
            SetStateDisabled();
            break;
        case(ErrorRecovery):
